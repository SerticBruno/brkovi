...
<main id="content" role="main">
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<header class="header">
			...
			<?php get_template_part( 'templates/wp/entry', 'meta' ); ?>
			<a href="<?php echo esc_url( get_permalink( $post->post_parent ) ); ?>" title="<?php printf( esc_attr__( 'Return to %s', 'brkovi' ), esc_attr( get_the_title( $post->post_parent ), 1 ) ); ?>" rev="attachment"><?php printf( esc_attr__( '%s Return to ', 'brkovi' ), '<span class="meta-nav">&larr;</span>' ); ?><?php echo wp_kses_post( get_the_title( $post->post_parent ) ); ?></a>
